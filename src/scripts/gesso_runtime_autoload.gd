...
# Gesso Runtime Autoload - WebSocket connection for game interaction
# Connects to Gesso MCP server on port 6505, accepting JSON commands over WebSocket.

var _server_url := "ws://127.0.0.1:6505"
...
var _visual_shader_cache: Dictionary = {}
var _next_visual_shader_id: int = 1
const RUNTIME_LOG_CAPACITY := 500
var _runtime_log_ring: Array = []
var _runtime_started_msec: int = 0

func _ready() -> void:
	_project_path = ProjectSettings.globalize_path("res://")
	_runtime_started_msec = Time.get_ticks_msec()
	process_mode = Node.PROCESS_MODE_ALWAYS
	_init_key_map()
	push_runtime_log("info", "GessoRuntime ready (project=%s)" % _project_path)
	
	# Check command line args for port override
	var args := OS.get_cmdline_args()
	for arg in args:
		if arg.begins_with("--mcp-port="):
			var custom_port := arg.substr(11).to_int()
			if custom_port > 0:
				_server_url = "ws://127.0.0.1:%d" % custom_port
				break
...
func _process(_delta: float) -> void:
	...
	if _busy and _busy_since > 0.0:
		...
		if elapsed > BUSY_TIMEOUT:
			push_warning("GessoRuntime: _busy flag stuck for %.1fs, force-resetting" % elapsed)
			_busy = false
			_busy_since = 0.0
	...
	if st == WebSocketPeer.STATE_OPEN:
		if not _connected:
			...
			print("GessoRuntime: Connected to Gesso MCP server.")

		while _socket.get_available_packet_count() > 0:
			var raw := _socket.get_packet().get_string_from_utf8()
			_handle_websocket_message(raw)
		...
		if _connected:
			...
			if close_code == 4001:
				print("GessoRuntime: Another runtime owns the MCP slot; retrying in 10s.")
				_reconnect_at_msec = Time.get_ticks_msec() + 10000
			else:
				print("GessoRuntime: Disconnected; will retry.")
		var now := Time.get_ticks_msec()
		if now >= _reconnect_at_msec:
			_attempt_connect()

func _attempt_connect() -> void:
	var prev_state := _socket.get_ready_state()
	if prev_state == WebSocketPeer.STATE_OPEN or prev_state == WebSocketPeer.STATE_CONNECTING:
		return
	_socket = WebSocketPeer.new()
	_socket.outbound_buffer_size = 16 * 1024 * 1024  # screenshots can be big
...
func _handle_websocket_message(json_string: String) -> void:
	...
	match msg_type:
		...
		"tool_invoke":
			...
			if not args is Dictionary:
				args = {}
			_handle_command(rid, tool_name, args)

func _handle_command(rid: String, command: String, params: Dictionary) -> void:
	if _busy:
		_send({
			"type": "tool_result",
			"id": rid,
			"success": false,
			"result": null,
			"error": "Server busy processing another command. Try again.",
		})
		return
	_busy = true
	...
	match command:
		...
		"input_sequence":
			await _cmd_input_sequence(params)
		"eval":
		...
		"query_runtime_node":
			_cmd_query_runtime_node(params)
		"get_runtime_log":
			_cmd_get_runtime_log(params)
...
func _send_response_raw(data: Dictionary) -> void:
	...
	_send({
		...
		"success": success,
		"result": result_data if success else null,
		"error": error_msg,
	})
...
# --- Runtime log ring (editor get_runtime_log compatibility) ---
func push_runtime_log(level: String, text: String) -> void:
	_runtime_log_ring.append({
		"ts_ms": Time.get_ticks_msec(),
		"level": level,
		"text": text,
	})
	if _runtime_log_ring.size() > RUNTIME_LOG_CAPACITY:
		_runtime_log_ring.pop_front()


func _cmd_get_runtime_log(params: Dictionary) -> void:
	var limit: int = clampi(int(params.get("limit", 200)), 1, RUNTIME_LOG_CAPACITY)
	var since_ms: int = int(params.get("since_ms", 0))
	var filtered: Array = []
	for entry in _runtime_log_ring:
	...
	_send_response({
		"success": true,
		"entries": filtered,
		"started_at_ms": _runtime_started_msec,
		"now_ms": Time.get_ticks_msec(),
	})


# --- Input sequence (automated playtesting) ---
func _cmd_input_sequence(params: Dictionary) -> void:
	var steps: Array = params.get("steps", [])
	if steps.is_empty():
		_send_response({"error": "steps must be a non-empty array"})
		return
	var stop_on_error: bool = bool(params.get("stop_on_error", true))
	var results: Array = []
	for i in range(steps.size()):
		var step_v: Variant = steps[i]
		if not step_v is Dictionary:
			var bad := {"index": i, "error": "each step must be an object"}
			results.append(bad)
			if stop_on_error:
				break
			continue
		var step: Dictionary = step_v
		var pre_wait_ms: int = int(step.get("wait_ms", 0))
		if pre_wait_ms > 0:
			await get_tree().create_timer(pre_wait_ms / 1000.0).timeout
		var outcome: Dictionary = await _run_input_sequence_step(i, step)
		results.append(outcome)
		if stop_on_error and outcome.has("error"):
			break
	_send_response({"success": true, "steps_executed": results.size(), "results": results})


func _run_input_sequence_step(index: int, step: Dictionary) -> Dictionary:
	var step_type: String = str(step.get("type", "")).to_lower()
	match step_type:
		"wait":
			var ms: int = int(step.get("duration_ms", step.get("wait_ms", 100)))
			await get_tree().create_timer(ms / 1000.0).timeout
			return {"index": index, "type": "wait", "ms": ms}
		"click":
			var err := await _sequence_click(step)
			if err.is_empty():
				return {"index": index, "type": "click", "x": step.get("x"), "y": step.get("y")}
			return {"index": index, "type": "click", "error": err}
		"key_press", "key":
			var err_key := await _sequence_key_press(step)
			if err_key.is_empty():
				return {"index": index, "type": "key_press", "key": step.get("key"), "action": step.get("action")}
			return {"index": index, "type": "key_press", "error": err_key}
		"key_hold":
			var err_hold := _sequence_key_hold(step)
			if err_hold.is_empty():
				return {"index": index, "type": "key_hold", "key": step.get("key"), "action": step.get("action")}
			return {"index": index, "type": "key_hold", "error": err_hold}
		"key_release":
			var err_rel := _sequence_key_release(step)
			if err_rel.is_empty():
				return {"index": index, "type": "key_release", "key": step.get("key"), "action": step.get("action")}
			return {"index": index, "type": "key_release", "error": err_rel}
		"mouse_move":
			_sequence_mouse_move(step)
			return {"index": index, "type": "mouse_move", "x": step.get("x"), "y": step.get("y")}
		"scroll":
			_sequence_scroll(step)
			return {"index": index, "type": "scroll", "direction": step.get("direction", "up")}
		"mouse_drag":
			var err_drag := await _sequence_mouse_drag(step)
			if err_drag.is_empty():
				return {"index": index, "type": "mouse_drag"}
			return {"index": index, "type": "mouse_drag", "error": err_drag}
		"send_input", "event":
			var event_desc: Dictionary = step.get("event", step)
			if not event_desc.has("type"):
				return {"index": index, "type": step_type, "error": "send_input step needs event.type"}
			var event: InputEvent = _build_input_event_from_desc(event_desc)
			if event == null:
...
func _sequence_click(step: Dictionary) -> String:
...
func _sequence_key_press(step: Dictionary) -> String:
	...
	if key.length() > 0:
		...
		Input.parse_input_event(release_event)
		return ""
	return "Must provide key or action"


func _sequence_key_hold(step: Dictionary) -> String:
	var action: String = str(step.get("action", ""))
	var key: String = str(step.get("key", ""))
	if action.length() > 0:
		Input.action_press(action)
...
func _sequence_key_release(step: Dictionary) -> String:
	var action: String = str(step.get("action", ""))
	var key: String = str(step.get("key", ""))
	if action.length() > 0:
		Input.action_release(action)
...
func _sequence_mouse_move(step: Dictionary) -> void:
	var x: float = float(step.get("x", 0))
	var y: float = float(step.get("y", 0))
	var relative_x: float = float(step.get("relative_x", 0))
	var relative_y: float = float(step.get("relative_y", 0))
...
func _sequence_scroll(step: Dictionary) -> void:
	var x: float = float(step.get("x", 0))
	var y: float = float(step.get("y", 0))
...
func _sequence_mouse_drag(step: Dictionary) -> String:
	var from_x: float = float(step.get("from_x", step.get("fromX", 0)))
	var from_y: float = float(step.get("from_y", step.get("fromY", 0)))
	var to_x: float = float(step.get("to_x", step.get("toX", 0)))
	var to_y: float = float(step.get("to_y", step.get("toY", 0)))
	var button: int = int(step.get("button", MOUSE_BUTTON_LEFT))
	var steps: int = maxi(int(step.get("steps", 10)), 1)
...
# --- send_input / query_runtime_node (editor MCP compatibility) ---
func _cmd_send_input(params: Dictionary) -> void:
	var event_desc: Dictionary = params.get("event", {})
	if event_desc.is_empty():
		_send_response({"error": "Missing 'event' dictionary"})
		return
	var event: InputEvent = _build_input_event_from_desc(event_desc)
	if event == null:
...
func _build_input_event_from_desc(desc: Dictionary) -> InputEvent:
	...
	match t:
		"key":
			...
			if desc.has("key"):
				k.physical_keycode = OS.find_keycode_from_string(str(desc["key"]).to_upper())
			return k
		"mouse_button":
			...
			if desc.has("position"):
				mb.position = _dict_to_vec2(desc["position"])
				mb.global_position = mb.position
			return mb
		"mouse_motion":
			var mm := InputEventMouseMotion.new()
			if desc.has("position"):
				mm.position = _dict_to_vec2(desc["position"])
...
func _cmd_query_runtime_node(params: Dictionary) -> void:
	...
	var include_children: bool = bool(params.get("include_children", false))
	var node: Node = get_tree().root.get_node_or_null(node_path)
	if node == null and not get_tree().current_scene == null:
		node = get_tree().current_scene.get_node_or_null(node_path)
	if node == null:
		_send_response({"error": "Node not found: %s" % node_path})
		return
	var info := {
	...
	for pname_v in properties:
		...
		if node.get(pname) != null:
			prop_values[pname] = _variant_to_json(node.get(pname))
...
func _default_visual_shader_stage(shader: VisualShader) -> int:
	match shader.mode:
		Shader.MODE_SKY:
...
func _cmd_visual_shader(params: Dictionary) -> void:
	...
	match action:
		"create":
			...
			var shader_type: String = str(params.get("shader_type", "spatial"))
			match shader_type:
				"spatial": shader.mode = Shader.MODE_SPATIAL
				"canvas_item": shader.mode = Shader.MODE_CANVAS_ITEM
				"particles": shader.mode = Shader.MODE_PARTICLES
				"sky": shader.mode = Shader.MODE_SKY
				"fog": shader.mode = Shader.MODE_FOG
				_:
					_send_response({"error": "Unknown shader_type: %s" % shader_type})
					return
			var sid: int = _next_visual_shader_id
			_next_visual_shader_id += 1
			_visual_shader_cache[sid] = shader
			_send_response({"success": true, "action": "create", "shader_id": sid, "shader_type": shader_type})
		"add_node":
			var sid: int = int(params.get("shader_id", 0))
			if not _visual_shader_cache.has(sid):
				_send_response({"error": "Unknown shader_id: %s" % sid})
				return
			var shader: VisualShader = _visual_shader_cache[sid]
			var node_class: String = str(params.get("node_class", ""))
			if node_class.is_empty():
				_send_response({"error": "node_class is required"})
				return
			if not ClassDB.class_exists(node_class):
				_send_response({"error": "Class not found: %s" % node_class})
				return
			var vs_node: Object = ClassDB.instantiate(node_class)
			if vs_node == null or not vs_node is VisualShaderNode:
				_send_response({"error": "Not a VisualShaderNode: %s" % node_class})
				return
			var vs_ref: VisualShaderNode = vs_node as VisualShaderNode
			var stage: int = int(params.get("shader_stage", _default_visual_shader_stage(shader)))
			var pos := Vector2.ZERO
			if params.has("position"):
				pos = _dict_to_vec2(params["position"])
			var node_id: int = shader.get_valid_node_id(stage)
			shader.add_node(stage, vs_ref, pos, node_id)
			_send_response({"success": true, "action": "add_node", "shader_id": sid, "node_id": node_id, "node_class": node_class, "shader_stage": stage})
		"connect":
			var sid: int = int(params.get("shader_id", 0))
			if not _visual_shader_cache.has(sid):
				_send_response({"error": "Unknown shader_id: %s" % sid})
			...
			shader.connect_nodes(
				stage,
				int(params.get("from_node", -1)),
				int(params.get("from_port", 0)),
				int(params.get("to_node", -1)),
				int(params.get("to_port", 0))
			)
			_send_response({"success": true, "action": "connect", "shader_id": sid})
		"disconnect":
			var sid: int = int(params.get("shader_id", 0))
			if not _visual_shader_cache.has(sid):
				_send_response({"error": "Unknown shader_id: %s" % sid})
			...
			_send_response({"success": true, "action": "disconnect", "shader_id": sid})
		"get_nodes":
			var sid: int = int(params.get("shader_id", 0))
			if not _visual_shader_cache.has(sid):
				_send_response({"error": "Unknown shader_id: %s" % sid})
			...
			var shader: VisualShader = _visual_shader_cache[sid]
			var nodes: Array = []
			for t in [VisualShader.TYPE_VERTEX, VisualShader.TYPE_FRAGMENT, VisualShader.TYPE_LIGHT, VisualShader.TYPE_SKY, VisualShader.TYPE_FOG]:
				for nid in shader.get_node_list(t):
					var vs_n = shader.get_node(t, nid)
					nodes.append({
						"id": nid,
						"shader_stage": t,
						"class": vs_n.get_class() if vs_n else "",
					})
			_send_response({"success": true, "action": "get_nodes", "shader_id": sid, "nodes": nodes})
		"apply":
			var sid: int = int(params.get("shader_id", 0))
			if not _visual_shader_cache.has(sid):
				_send_response({"error": "Unknown shader_id: %s" % sid})
			...
			mat.shader = _visual_shader_cache[sid]
			if target is GeometryInstance3D:
				(target as GeometryInstance3D).material_override = mat
			elif target is CanvasItem:
				(target as CanvasItem).material = mat
			else:
				_send_response({"error": "Node does not support materials: %s" % target.get_class()})
				return
			_send_response({"success": true, "action": "apply", "shader_id": sid, "node_path": node_path})
		_:
			_send_response({"error": "Unknown visual_shader action: %s" % action})
...
func _cmd_terrain(params: Dictionary) -> void:
	...
	match action:
		"create":
			var parent_path: String = str(params.get("parent_path", "/root"))
			var parent: Node = get_tree().root.get_node_or_null(parent_path)
			if parent == null:
				_send_response({"error": "Parent not found: %s" % parent_path})
				return
			var width: int = maxi(int(params.get("width", 8)), 1)
			var depth: int = maxi(int(params.get("depth", 8)), 1)
			var max_height: float = float(params.get("max_height", 10.0))
			var heights: PackedFloat32Array = _terrain_heights_from_params(params, width, depth)
			var mesh: ArrayMesh = _build_terrain_mesh(width, depth, heights, max_height)
			...
			_send_response({
				...
				"width": width,
				"depth": depth,
				"vertex_count": (width + 1) * (depth + 1),
			})
		"modify":
			_apply_terrain_brush(params, false)
		"paint":
			_apply_terrain_brush(params, true)
		"get_height":
			var node_path: String = str(params.get("node_path", ""))
			var mi: MeshInstance3D = get_tree().root.get_node_or_null(node_path) as MeshInstance3D
			if mi == null or mi.mesh == null:
				_send_response({"error": "Terrain MeshInstance3D not found: %s" % node_path})
				return
			var x: int = int(params.get("x", 0))
			var z: int = int(params.get("z", 0))
			var h: float = _terrain_sample_height(mi, x, z)
			_send_response({"success": true, "action": "get_height", "x": x, "z": z, "height": h})
		_:
			_send_response({"error": "Unknown terrain action: %s" % action})


func _terrain_heights_from_params(params: Dictionary, width: int, depth: int) -> PackedFloat32Array:
	var count: int = (width + 1) * (depth + 1)
	var heights := PackedFloat32Array()
	heights.resize(count)
	var raw: Array = params.get("height_data", [])
	for i in range(mini(raw.size(), count)):
		heights[i] = float(raw[i])
	return heights


func _build_terrain_mesh(width: int, depth: int, heights: PackedFloat32Array, max_height: float) -> ArrayMesh:
	var verts := PackedVector3Array()
	var uvs := PackedVector2Array()
	var indices := PackedInt32Array()
	for z in range(depth + 1):
		for x in range(width + 1):
			var idx: int = z * (width + 1) + x
			var h: float = heights[idx] if idx < heights.size() else 0.0
			verts.append(Vector3(float(x), h * max_height, float(z)))
			uvs.append(Vector2(float(x) / float(width), float(z) / float(depth)))
	for z in range(depth):
		for x in range(width):
			var i: int = z * (width + 1) + x
			indices.append_array([i, i + width + 1, i + 1, i + 1, i + width + 1, i + width + 2])
	var arrays: Array = []
	arrays.resize(Mesh.ARRAY_MAX)
	arrays[Mesh.ARRAY_VERTEX] = verts
	arrays[Mesh.ARRAY_TEX_UV] = uvs
...
func _terrain_mesh_arrays(mi: MeshInstance3D) -> Array:
	if mi.mesh == null or mi.mesh.get_surface_count() == 0:
		return []
	return mi.mesh.surface_get_arrays(0)


func _terrain_sample_height(mi: MeshInstance3D, x: int, z: int) -> float:
	var arrays: Array = _terrain_mesh_arrays(mi)
	if arrays.is_empty() or arrays[Mesh.ARRAY_VERTEX] == null:
		return 0.0
	var verts: PackedVector3Array = arrays[Mesh.ARRAY_VERTEX]
	var gx: int = clampi(x, 0, verts.size() - 1)
	# Approximate grid lookup by x/z — assumes grid topology from create
	for v in verts:
		if int(v.x) == x and int(v.z) == z:
			return v.y
	return 0.0


func _apply_terrain_brush(params: Dictionary, paint_color: bool) -> void:
	var node_path: String = str(params.get("node_path", ""))
	var mi: MeshInstance3D = get_tree().root.get_node_or_null(node_path) as MeshInstance3D
	if mi == null or mi.mesh == null:
		_send_response({"error": "Terrain MeshInstance3D not found: %s" % node_path})
	...
	var arrays: Array = _terrain_mesh_arrays(mi)
	if arrays.is_empty():
		_send_response({"error": "Terrain has no mesh surface"})
		return
	var verts: PackedVector3Array = arrays[Mesh.ARRAY_VERTEX]
	var cx: float = float(params.get("x", 0))
	var cz: float = float(params.get("z", 0))
	var radius: float = float(params.get("radius", 1.0))
	var delta: float = float(params.get("height_delta", 0.5))
	for i in range(verts.size()):
		var v: Vector3 = verts[i]
		var dist: float = Vector2(v.x - cx, v.z - cz).length()
		if dist <= radius:
			if paint_color:
				pass  # vertex colors optional; height brush only for now
			else:
				verts[i] = Vector3(v.x, v.y + delta, v.z)
	arrays[Mesh.ARRAY_VERTEX] = verts
	var mesh := ArrayMesh.new()
	mesh.add_surface_from_arrays(Mesh.PRIMITIVE_TRIANGLES, arrays)
	mi.mesh = mesh
	_send_response({"success": true, "action": "modify" if not paint_color else "paint", "node_path": node_path})
...
func _cmd_video(params: Dictionary) -> void:
	...
	match action:
		"create":
			...
			var video_path: String = str(params.get("video_path", ""))
			if not video_path.is_empty():
				if ResourceLoader.exists(video_path):
					player.stream = load(video_path) as VideoStream
				else:
					_send_response({"error": "Video not found: %s" % video_path})
					return
			if params.has("volume"):
				player.volume_db = linear_to_db(clampf(float(params["volume"]), 0.0, 1.0))
			...
			if bool(params.get("autoplay", false)):
				player.play()
			_send_response({"success": true, "action": "create", "path": str(player.get_path())})
		"play", "pause", "stop", "seek":
			var node_path: String = str(params.get("node_path", ""))
			if node_path.is_empty():
				_send_response({"error": "node_path is required for video %s" % action})
				return
			var player: VideoStreamPlayer = get_tree().root.get_node_or_null(node_path) as VideoStreamPlayer
			if player == null:
			...
			match action:
				"play":
					player.play()
					_send_response({"success": true, "action": "play", "playing": player.is_playing()})
				"pause":
					player.stream_paused = true
					_send_response({"success": true, "action": "pause", "playing": player.is_playing()})
				"stop":
					player.stop()
					_send_response({"success": true, "action": "stop", "playing": false})
				"seek":
				...
				_:
					_send_response({"error": "Unknown video control action: %s" % action})
		"get_status":
			var status_path: String = str(params.get("node_path", ""))
			if status_path.is_empty():
				_send_response({"error": "node_path is required for video get_status"})
				return
			var status_player: VideoStreamPlayer = get_tree().root.get_node_or_null(status_path) as VideoStreamPlayer
			if status_player == null:
				_send_response({"error": "VideoStreamPlayer not found: %s" % status_path})
				return
			# Headless runs may not have video backends; avoid stream APIs that can block.
			_send_response({
				"success": true,
				"action": "get_status",
				"playing": status_player.is_playing(),
				"paused": status_player.is_paused(),
				"position": status_player.get_stream_position(),
			})
		_:
