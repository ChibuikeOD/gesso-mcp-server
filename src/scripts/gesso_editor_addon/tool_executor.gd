...
func _init_tools() -> void:
	...
	if _initialized and _tool_map.size() > 0:
		return
	if _initialized and _tool_map.is_empty():
		push_warning("[Gesso MCP] Tool map was empty; re-initializing handlers.")
		_initialized = false
	...
	_tool_map = {
		...
		&"configure_game_viewport": [_project_tools, &"configure_game_viewport"],
		&"capture_screen": [_project_tools, &"capture_screen"],
...
func execute_tool(tool_name: String, args: Dictionary) -> Dictionary:
	...
	var lookup_key := StringName(tool_name)
	if not _tool_map.has(lookup_key):
		var available: PackedStringArray = PackedStringArray()
		for k in _tool_map.keys():
			available.append(str(k))
		return {
			&"ok": false,
			&"error": "Unknown tool: %s. Available: %s" % [tool_name, ", ".join(available)]
		}

	var handler: Array = _tool_map[lookup_key]
