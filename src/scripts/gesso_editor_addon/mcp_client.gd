...
func _handle_disconnect() -> void:
	...
	print("[MCP] Disconnected from server (code: ", close_code, ")")
	disconnected.emit()

	if _should_reconnect:
		if close_code == 4000:
			print("[MCP] Another editor owns the Gesso MCP slot; backing off reconnect to 10s.")
			if _reconnect_timer:
				_reconnect_timer.start(10.0)
		else:
			_schedule_reconnect()
