...
##          list_viewport_presets, configure_game_viewport, capture_screen,
...
# =============================================================================
func list_viewport_presets(_args: Dictionary) -> Dictionary:
	return {&"ok": true, &"presets": _viewport_preset_catalog()}

# =============================================================================
# configure_game_viewport
# =============================================================================
func configure_game_viewport(args: Dictionary) -> Dictionary:
	var preset: String = str(args.get(&"preset", "hd_720p")).strip_edges()
	var stretch_mode: String = str(args.get(&"stretch_mode", "canvas_items")).strip_edges()
	var stretch_aspect: String = str(args.get(&"stretch_aspect", "expand")).strip_edges()
	var resizable: bool = bool(args.get(&"resizable", true))

	var catalog: Dictionary = _viewport_preset_catalog()
	var window_w: int
	var window_h: int
	var viewport_w: int
	var viewport_h: int
	...
	if preset == "custom":
		window_w = int(args.get(&"width", 1280))
		window_h = int(args.get(&"height", 720))
		if window_w < 160 or window_h < 120:
			return {&"ok": false, &"error": "Custom width/height too small (min 160x120)."}
		viewport_w = int(args.get(&"design_width", window_w))
		viewport_h = int(args.get(&"design_height", window_h))
		label = "%dx%d (custom)" % [window_w, window_h]
	elif catalog.has(preset):
		var info: Dictionary = catalog[preset]
		window_w = int(info[&"window_width"])
		window_h = int(info[&"window_height"])
		viewport_w = int(info.get(&"design_width", window_w))
		viewport_h = int(info.get(&"design_height", window_h))
		...
		return {
			&"ok": false,
			&"error": "Unknown preset '%s'. Use list_viewport_presets." % preset,
			&"available_presets": keys
		}

	_apply_viewport_settings(viewport_w, viewport_h, window_w, window_h, stretch_mode, stretch_aspect, resizable)
	_save_and_refresh_settings()

	var upscaled: bool = viewport_w != window_w or viewport_h != window_h
	return {
		&"ok": true,
		&"preset": preset,
		&"label": label,
...
func _viewport_preset_catalog() -> Dictionary:
	return {
		"hd_720p": {
			&"window_width": 1280, &"window_height": 720,
			&"design_width": 400, &"design_height": 256,
			&"label": "1280x720 window, 400x256 design (upscaled 2D indie)",
			&"aspect": "16:9",
		},
		"hd_1080p": {
			&"window_width": 1920, &"window_height": 1080,
			&"design_width": 400, &"design_height": 256,
			&"label": "1920x1080 window, 400x256 design (upscaled)",
			&"aspect": "16:9",
		},
		"classic_960": {
			&"window_width": 960, &"window_height": 540,
			&"design_width": 400, &"design_height": 256,
			&"label": "960x540 window, 400x256 design (upscaled)",
			&"aspect": "16:9",
		},
		"pixel_640": {
			&"window_width": 640, &"window_height": 360,
			&"design_width": 400, &"design_height": 256,
			&"label": "640x360 window, 400x256 design (upscaled)",
			&"aspect": "16:9",
		},
	}

func _apply_viewport_settings(
	viewport_w: int, viewport_h: int, window_w: int, window_h: int,
	stretch_mode: String, stretch_aspect: String, resizable: bool
) -> void:
	ProjectSettings.set_setting("display/window/size/viewport_width", viewport_w)
	ProjectSettings.set_setting("display/window/size/viewport_height", viewport_h)
	if window_w > 0 and window_h > 0 and (window_w != viewport_w or window_h != viewport_h):
		ProjectSettings.set_setting("display/window/size/window_width_override", window_w)
		ProjectSettings.set_setting("display/window/size/window_height_override", window_h)
	else:
	...
	ProjectSettings.set_setting("display/window/size/resizable", resizable)
...
# =============================================================================
# capture_screen — game / editor / fullscreen PNG captures for agent vision
# =============================================================================
func capture_screen(args: Dictionary) -> Dictionary:
	# Wait one frame so viewports finish drawing before readback.
	if Engine.get_main_loop():
		await Engine.get_main_loop().process_frame
	return _capture_screen_impl(args)

func _capture_screen_impl(args: Dictionary) -> Dictionary:
	var target: String = str(args.get(&"target", "game")).strip_edges().to_lower()
	var save_to: String = str(args.get(&"save_to", "")).strip_edges()
	var return_base64: bool = bool(args.get(&"return_base64", true))
	var screen_index: int = int(args.get(&"screen_index", 0))

	match target:
		"game", "runtime", "play", "debug":
			return _capture_game_window(save_to, return_base64)
		"editor", "engine", "viewport":
			return _capture_editor_view(save_to, return_base64)
		"fullscreen", "desktop", "monitor", "screen":
			return _capture_fullscreen(save_to, return_base64, screen_index)
		_:
			return {
				&"ok": false,
				&"error": "Unknown target '%s'. Use: game, editor, or fullscreen." % target,
				&"targets": ["game", "editor", "fullscreen"],
			}

func _capture_game_window(save_to: String, return_base64: bool) -> Dictionary:
	...
	if not ei.is_playing_scene():
		return {
			&"ok": false,
			&"error": "Game is not running. Call run_scene first, then capture_screen with target=game.",
		}

	var window_id := _find_debug_game_window_id()
	if window_id == -1:
		return {&"ok": false, &"error": "Could not find the game debug window. Is the game running?"}

	if not DisplayServer.has_method(&"window_get_image"):
		return {&"ok": false, &"error": "DisplayServer.window_get_image is unavailable in this Godot build."}

	var img_variant: Variant = DisplayServer.call(StringName("window_get_image"), window_id)
	var img := img_variant as Image
	if img == null or img.is_empty():
		return {&"ok": false, &"error": "Failed to read game window image (window_id=%d)." % window_id}

	var window_title := str(DisplayServer.call(StringName("window_get_title"), window_id))
	return _finalize_capture(img, save_to, return_base64, "game", {
		&"window_id": window_id,
		&"window_title": window_title,
	})

func _capture_editor_view(save_to: String, return_base64: bool) -> Dictionary:
	if not _editor_plugin:
		return {&"ok": false, &"error": "Editor plugin not available"}
	var ei := _editor_plugin.get_editor_interface()
	var img: Image = null
	var source := "editor_base"

	var vp2d: Node = ei.get_editor_viewport_2d()
	if vp2d != null and vp2d is CanvasItem:
		var tex: ViewportTexture = vp2d.get_viewport().get_texture()
		if tex:
			img = tex.get_image()
	...
	if img == null or img.is_empty():
		...
		if base:
			var tex2: ViewportTexture = base.get_viewport().get_texture()
			if tex2:
				img = tex2.get_image()
				source = "editor_base"

	if img == null or img.is_empty():
		return {&"ok": false, &"error": "Could not capture the Godot editor viewport."}

	return _finalize_capture(img, save_to, return_base64, "editor", {&"source": source})

func _capture_fullscreen(save_to: String, return_base64: bool, screen_index: int) -> Dictionary:
	if screen_index < 0 or screen_index >= DisplayServer.get_screen_count():
		return {
			&"ok": false,
			&"error": "Invalid screen_index %d (screens: %d)." % [screen_index, DisplayServer.get_screen_count()],
		}
	var img: Image = DisplayServer.screen_get_image(screen_index)
	if img == null or img.is_empty():
		return {&"ok": false, &"error": "screen_get_image returned empty for screen %d." % screen_index}

	return _finalize_capture(img, save_to, return_base64, "fullscreen", {
...
func _find_debug_game_window_id() -> int:
	...
	for wid: int in DisplayServer.get_window_list():
		var title := str(DisplayServer.call(StringName("window_get_title"), wid))
		if title.find("(DEBUG)") >= 0:
			return wid
		if not project_name.is_empty() and title.find(project_name) >= 0 and title != main_title:
			return wid
	return -1

func _finalize_capture(
	img: Image, save_to: String, return_base64: bool, target: String, extra: Dictionary
) -> Dictionary:
	if not MCPPaths.ensure_screenshot_cache_dir():
		return {&"ok": false, &"error": "Could not create screenshot cache directory."}

	var resource_path := save_to
	if resource_path.is_empty():
		resource_path = "%scapture_%s_%d.png" % [
			MCPPaths.SCREENSHOT_CACHE_DIR, target, Time.get_ticks_msec()
		]
	...
	if err != OK:
		return {
			&"ok": false,
			&"error": "save_png failed: %s at %s" % [error_string(err), abs_path],
		}
	...
	out.merge(extra)
	if return_base64:
		out[&"base64_png"] = Marshalls.raw_to_base64(img.save_png_to_buffer())
	return out
