...
func _init():
    ...
    match operation:
        ...
        "list_viewport_presets":
            list_viewport_presets(params)
        "configure_game_viewport":
            configure_game_viewport(params)
        "capture_screen":
...
func validate_script(params):
    ...
    log_info("Validating script: " + full_path)

    if not FileAccess.file_exists(full_path):
        printerr("Script file does not exist at: " + full_path)
        quit(1)

    var file = FileAccess.open(full_path, FileAccess.READ)
    if not file:
        printerr("Cannot read file: " + full_path)
        quit(1)
    ...
    if err != OK:
        output["message"] = "Script has syntax errors (Error code: " + str(err) + ")"
    elif not script.can_instantiate():
        output["valid"] = false
        output["message"] = "Script parsed but cannot be instantiated (dependency errors)"
    else:
        output["message"] = "Script is valid"

    print("VALIDATION_JSON_START")
    print(JSON.stringify(output))
    print("VALIDATION_JSON_END")

func list_viewport_presets(_params) -> void:
    var output := {
        "ok": true,
        "presets": {
            "hd_720p": {"window_width": 1280, "window_height": 720, "design_width": 400, "design_height": 256, "label": "1280x720 window, 400x256 design (upscaled 2D indie)", "aspect": "16:9"},
            "hd_1080p": {"window_width": 1920, "window_height": 1080, "design_width": 400, "design_height": 256, "label": "1920x1080 window, 400x256 design (upscaled)", "aspect": "16:9"},
            "classic_960": {"window_width": 960, "window_height": 540, "design_width": 400, "design_height": 256, "label": "960x540 window, 400x256 design (upscaled)", "aspect": "16:9"},
            "pixel_640": {"window_width": 640, "window_height": 360, "design_width": 400, "design_height": 256, "label": "640x360 window, 400x256 design (upscaled)", "aspect": "16:9"},
        }
    }
    print("VIEWPORT_JSON_START")
    print(JSON.stringify(output))
    print("VIEWPORT_JSON_END")

func configure_game_viewport(params) -> void:
    var preset: String = str(params.get("preset", "hd_720p"))
    var stretch_mode: String = str(params.get("stretch_mode", "canvas_items"))
    var stretch_aspect: String = str(params.get("stretch_aspect", "expand"))
    var resizable: bool = bool(params.get("resizable", true))

    var catalog := {
        "hd_720p": {"window_width": 1280, "window_height": 720, "design_width": 400, "design_height": 256},
        "hd_1080p": {"window_width": 1920, "window_height": 1080, "design_width": 400, "design_height": 256},
        "classic_960": {"window_width": 960, "window_height": 540, "design_width": 400, "design_height": 256},
        "pixel_640": {"window_width": 640, "window_height": 360, "design_width": 400, "design_height": 256},
    }
    ...
    if preset == "custom":
        window_w = int(params.get("width", 1280))
        window_h = int(params.get("height", 720))
        viewport_w = int(params.get("design_width", window_w))
        viewport_h = int(params.get("design_height", window_h))
        ...
        label = "%dx%d window, %dx%d design" % [window_w, window_h, viewport_w, viewport_h]
    else:
        printerr("Unknown preset: " + preset)
        quit(1)
    ...
    if window_w != viewport_w or window_h != viewport_h:
    ...
    var output := {
        ...
        "upscaled_from_design": viewport_w != window_w or viewport_h != window_h,
...
func capture_screen(params) -> void:
    var target: String = str(params.get("target", "fullscreen")).to_lower()
    if target != "fullscreen" and target != "desktop" and target != "monitor" and target != "screen":
        printerr("Headless capture_screen only supports target=fullscreen (game/editor need the Godot editor plugin).")
        quit(1)
    ...
    if screen_count < 1:
        var output := {
            "ok": false,
            "target": "fullscreen",
            "error": "No displays available in headless Godot. Use capture_screen from the editor plugin (game/editor targets) while Godot is open.",
        }
        print("CAPTURE_JSON_START")
        print(JSON.stringify(output))
        print("CAPTURE_JSON_END")
        return
    var screen_index: int = int(params.get("screen_index", 0))
    if screen_index < 0 or screen_index >= screen_count:
        printerr("Invalid screen_index %d (screens: %d)" % [screen_index, screen_count])
        quit(1)
    var img: Image = DisplayServer.screen_get_image(screen_index)
    if img == null or img.is_empty():
        printerr("screen_get_image failed")
        quit(1)
    var save_to: String = str(params.get("save_to", ""))
    var resource_path := save_to
    if resource_path.is_empty():
        resource_path = "res://addons/godot_mcp/cache/screenshots/capture_fullscreen_%d.png" % Time.get_ticks_msec()
    ...
    if bool(params.get("return_base64", true)):
        output["base64_png"] = Marshalls.raw_to_base64(img.save_png_to_buffer())
    print("CAPTURE_JSON_START")
    print(JSON.stringify(output))
    print("CAPTURE_JSON_END")
