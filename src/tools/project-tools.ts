...
export const projectTools: ToolDefinition[] = [
  ...
  {
    name: 'list_viewport_presets',
    description:
      'List built-in 2D PC indie viewport presets for configure_game_viewport (resolution, aspect ratio, and recommended use).',
    inputSchema: {
      type: 'object',
      properties: {},
    },
  ...
  {
    name: 'configure_game_viewport',
    description:
      'Set the game window/viewport to a standard 2D PC indie resolution with sensible stretch defaults. Presets: hd_720p (1280x720, default), hd_1080p (1920x1080), classic_960 (960x540), pixel_640 (640x360). Use preset "custom" with width/height for other sizes. Applies display/window/size/viewport_*, stretch mode canvas_items, and aspect expand (letterbox-safe scaling). Saves project.godot.',
    inputSchema: {
      type: 'object',
      properties: {
        preset: {
          type: 'string',
          description:
            'Preset name: hd_720p | hd_1080p | classic_960 | pixel_640 | custom (default: hd_720p)',
        },
        width: {
          type: 'number',
          description: 'Custom viewport width when preset is "custom" (default: 1280)',
        },
        height: {
          type: 'number',
          description: 'Custom viewport height when preset is "custom" (default: 720)',
        },
        stretch_mode: {
          type: 'string',
          description:
            'Godot stretch mode (default: canvas_items). Use canvas_items for 2D games.',
        },
        stretch_aspect: {
          type: 'string',
          description:
            'Godot stretch aspect: expand | keep | keep_width | keep_height (default: expand)',
        },
        resizable: {
          type: 'boolean',
          description: 'Allow resizing the game window on PC (default: true)',
        },
      },
    },
  },
  ...
  },
  {
    name: 'capture_screen',
    description:
      'Capture a PNG for agent vision while debugging display/layout issues. Targets: "game" = running game debug window (F5; use after run_scene); "editor" = Godot editor 2D/main viewport; "fullscreen" = entire monitor/desktop. Returns resource_path, absolute_path, width, height, and base64_png (default true). Saves to res://addons/godot_mcp/cache/screenshots/ by default.',
    inputSchema: {
      type: 'object',
      properties: {
        target: {
          type: 'string',
          description: 'Capture target: game | editor | fullscreen (default: game)',
        },
        save_to: {
          type: 'string',
          description: 'Optional res:// or user:// PNG path',
        },
        return_base64: {
          type: 'boolean',
          description: 'Include base64_png in the response (default: true)',
        },
        screen_index: {
          type: 'number',
          description: 'Monitor index for target=fullscreen (default: 0)',
        },
      },
    },
  },
  ...
  {
    name: 'test_input_sequence',
    description: 'Run a scripted list of inputs in the **running game** (clicks, keys, scroll, drag, waits). Same as game_input_sequence. Requires play mode + GessoRuntime. Example: {steps:[{type:"wait",duration_ms:300},{type:"key_press",key:"ui_right"},{type:"click",x:200,y:300}]}. Pair with take_screenshot or query_runtime_node to verify behavior.',
    inputSchema: {
      type: 'object',
      properties: {
        steps: {
          type: 'array',
          items: { type: 'object' },
          description: 'Steps: wait | click | key_press | key_hold | key_release | mouse_move | scroll | mouse_drag | send_input',
        },
